package com.fwang.utils
{
	...
	public final class JPEGEncoder
	{
		...
		private function initQuantTables(sf:int):void
		{
			...
			{
				var u:int = int((UVQT[j]*sf+50)*0.01);
				if (u < 1) {
					u = 1;
				} else if (u > 255) {
					u = 255;
		...
		private function init():void
		{
			...
			for (i = 0; i<lng; ++i) YQT[i] = YQTBuffer[i];
			// uvqt
			lng = UVQTBuffer.length;
			for (i = 0; i<lng; ++i) UVQT[i] = UVQTBuffer[i];
			// zigzag
			lng = ZigZagBuffer.length;
			for (i = 0; i<lng; ++i) ZigZag[i] = ZigZagBuffer[i];
			// luminance
			lng = std_dc_luminance_nrcodesBuffer.length;
			for (i = 0; i<lng; ++i) std_dc_luminance_nrcodes[i] = std_dc_luminance_nrcodesBuffer[i];
			lng = std_dc_luminance_valuesBuffer.length;
			for (i = 0; i<lng; ++i) std_dc_luminance_values[i] = std_dc_luminance_valuesBuffer[i];
			lng = std_ac_luminance_nrcodesBuffer.length;
			for (i = 0; i<lng; ++i) std_ac_luminance_nrcodes[i] = std_ac_luminance_nrcodesBuffer[i];
			lng = std_ac_luminance_valuesBuffer.length;
			for (i = 0; i<lng; ++i) std_ac_luminance_values[i] = std_ac_luminance_valuesBuffer[i];
			// chrominance
			lng = std_dc_chrominance_nrcodesBuffer.length;
			for (i = 0; i<lng; ++i) std_dc_chrominance_nrcodes[i] = std_dc_chrominance_nrcodesBuffer[i];
			lng = std_dc_chrominance_valuesBuffer.length;
			for (i = 0; i<lng; ++i) std_dc_chrominance_values[i] = std_dc_chrominance_valuesBuffer[i];
			lng = std_ac_chrominance_nrcodesBuffer.length;
			for (i = 0; i<lng; ++i) std_ac_chrominance_nrcodes[i] = std_ac_chrominance_nrcodesBuffer[i];
			lng = std_ac_chrominance_valuesBuffer.length;
			for (i = 0; i<lng; ++i) std_ac_chrominance_values[i] = std_ac_chrominance_valuesBuffer[i];
			// Create tables
			initHuffmanTbl();
			initCategoryNumber();
			initQuantTables(sf);
